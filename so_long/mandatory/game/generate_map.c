...
/*   Updated: 2025/01/20 14:08:13 by jotudela         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int generate_textures(t_data *data)
{
    ...
    data->img.exit1 = mlx_xpm_file_to_image(data->mlx, EXIT1, &data->img.x, &data->img.y);
    data->img.exit2 = mlx_xpm_file_to_image(data->mlx, EXIT2, &data->img.x, &data->img.y);
    data->len_lst = ft_lstlen(*data->map);
    data->actions = 0;
    if (!data->img.wall || !data->img.tree || !data->img.floor || !data->img.link || !data->img.triforce || !data->len_lst || !data->img.exit1 || !data->img.exit2)
        return (1);
    return (0);
}
...
void    print_map(t_data *data, t_map *map)
{
    ...
    while (map)
    {
        ...
        while (x < map->len_line && map->line[x])
        {
            ...
            else if (map->line[x] == '1' && (x > 0 && x < map->len_line - 1) && (y > 0 && y < data->len_lst - 1))
