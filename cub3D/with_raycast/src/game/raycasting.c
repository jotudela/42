...
/*   Updated: 2025/05/06 15:46:07 by jotudela         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void raycasting(t_data *data)
{
    ...
    {
        float cameraX = 2 * x / (float)data->img.img_x - 1;
        ...
        while (hit == 0)
        {
            ...
            if (mapY < 0 || mapY >= data->map->len_y ||
                mapX < 0 || mapX >= data->map->len_x ||
                data->map->tab[mapY][mapX] == '1' ||
                data->map->tab[mapY][mapX] == ' ')
            {
                hit = 1;
            }
        }
        ...
        draw_walls(data, x, drawStart, drawEnd, color);
    }
